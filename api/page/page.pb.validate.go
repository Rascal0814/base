...
// ValidateAll checks the field values on PageCtx with the rules defined in the
// proto definition for this message. If any rules are violated, the result is
// a list of violation errors wrapped in PageCtxMultiError, or nil if none found.
func (m *PageCtx) ValidateAll() error {
	return m.validate(true)
}

func (m *PageCtx) validate(all bool) error {
	if m == nil {
		return nil
	}
	...
	if len(errors) > 0 {
		return PageCtxMultiError(errors)
	}

	return nil
}

// PageCtxMultiError is an error wrapping multiple validation errors returned
// by PageCtx.ValidateAll() if the designated constraints aren't met.
type PageCtxMultiError []error

// Error returns a concatenation of all the error messages it wraps.
func (m PageCtxMultiError) Error() string {
	var msgs []string
	for _, err := range m {
		msgs = append(msgs, err.Error())
	}
...
// AllErrors returns a list of validation violation errors.
func (m PageCtxMultiError) AllErrors() []error { return m }
